...
# SciencePlots: 一键SCI期刊级图表美化
try:
    import scienceplots
    plt.style.use(['science', 'no-latex'])  # 无需LaTeX的SCI风格
    print("[style] SciencePlots loaded: science/no-latex ✅")
except ImportError:
...
def save(fig, name): 
    ...
    fig.savefig(os.path.join(OUT, name), bbox_inches="tight", facecolor="white", dpi=300)
...
def fig2():
    fig,ax=plt.subplots(figsize=(10,6.5),dpi=300); ax.set_xlim(0,12); ax.set_ylim(0,10); ax.axis("off")
...
def fig3():
    ...
    ax.axhline(0.8,ls=":",color="gray",lw=0.8); ax.text(0.6,0.81,"Hj=0.8",fontsize=7,color="gray")
    save(fig,"Fig4_perclass.png")
...
def fig4():
    ...
    save(fig,"Fig5_multitemporal.png")
...
def fig5():
    ...
    ax.text(0.5,0.66,f"$\\Delta$HQ = {means[0]-means[3]:.3f} (41.9% of mean)",color="#7b241c",fontsize=10)
    ax.set_xticks(range(4)); ax.set_xticklabels(["Q1\n(low)","Q2","Q3","Q4\n(high)"])
    ax.set_xlabel("Built-up threat exposure quartile"); ax.set_ylabel("Mean forest HQ")
    ax.set_title("Forest HQ vs built-up threat (identical Hj=0.80)"); ax.set_ylim(0.4,0.8)
    save(fig,"Fig6_quartiles.png")
...
def fig6():
    ...
    st={0:("#cfd8dc","Not significant",4),1:("#d32f2f","HH (eco-core)",8),2:("#1565c0","LL (threat hotspot)",8),
        3:("#fb8c00","HL (stepping stone)",10),4:("#4dd0e1","LH (edge)",10)}
    for k in [0,1,2,4,3]:
    ...
    ax.legend(loc="upper left",fontsize=8,title="LISA cluster",title_fontsize=8,framealpha=0.9)
    save(fig,"Fig7_lisa.png")
...
def fig8():
    ...
    plt.tight_layout(); save(fig,"Fig9_scenario.png")
...
def fig9():
    ...
    save(fig,"Fig10_ksensitivity.png")
...
def figS1():
    ...
    ax.set_title("Monte Carlo robustness of $\eta^2$ (never below ~0.83)"); ax.legend(fontsize=9)
